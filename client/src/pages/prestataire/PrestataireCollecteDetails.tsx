/**
 * Tape'a Prestataire - Page Détails Collecte
 * Affiche le détail des courses d'une collecte
 */

import { useEffect, useState } from 'react';
import { Link, useParams, useLocation } from 'wouter';
import { ArrowLeft, User, Calendar, CreditCard, Banknote, Check, Clock, Info } from 'lucide-react';
...
export function PrestataireCollecteDetails() {
  ...
  async function fetchDetails() {
    ...
    try {
      ...
      const response = await fetch(`/api/prestataire/collecte/${params.id}`, {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            Détail - {formatPeriode(data.collecte.periode)}
          </h1>
          {data.driver && (
            <div className="flex items-center gap-2 text-gray-600">
              <User className="h-4 w-4" />
              <span>Chauffeur: {data.driver.firstName} {data.driver.lastName}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="rounded-lg bg-white p-4 shadow">
          <div className={`text-2xl font-bold ${data.collecte.isPaid ? 'text-green-600' : 'text-orange-600'}`}>
            {data.collecte.isPaid ? 'Payé' : 'À payer'}
          </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {data.courses.map((course) => (
                  <tr
                    key={course.id}
                    onClick={() => setLocation(`/prestataire/courses/${course.id}`)}
